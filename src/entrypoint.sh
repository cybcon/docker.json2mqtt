#!/bin/bash
##############################################################################
# HTTP JSON endpoint collector
# The script will get data from a HTTP endpoint that provides a json and
# send this json payload inside a MQTT topic
#-----------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------
# get_http_json_endpoint
# @desc: trigger curl to get the json data from the http endpoint
# @return: string, json output
#-----------------------------------------------------------------------------
function get_http_json_endpoint {
  endpoint="${1}"

  if [ -z "${endpoint}" ]; then
    echo "ERROR: Internal error in function get_http_json_endpoint! Required parameter missing!" >&2
...
echo "JSON payload collector v${VERSION} started"

validate_input_parameters

echo "Get json payload from: ${ENDPOINT}"
result=$(get_http_json_endpoint ${ENDPOINT})
publish_result "${result}"

echo "JSON payload collector v${VERSION} stopped"
